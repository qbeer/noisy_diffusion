...
class ConditionalInstanceNorm2d(nn.Module):
    ...
    blob/master/models/cond_refinenet_dilated.py#L48
    """
    ...
    def __init__(self, num_features, num_classes, bias=True):
        ...
        self.bias = bias
        # affine was set originally to False which I think is wrong based
    ...
    def forward(self, x, y):
        ...
        if self.bias:
            ...
            out = gamma.view(-1, self.num_features, 1, 1) * \
                h + beta.view(-1, self.num_features, 1, 1)
        else:
            gamma = self.embed(y)
...
class ConditionalInstanceNormPP(nn.Module):
    def __init__(self, in_features, in_labels, eps=1e-5):
        ...
        self.alpha = nn.Parameter(
            torch.ones(
                size=(in_labels, in_features, 1, 1),
            ),
        )
    ...
    def forward(self, x, y):
        ...
        # [bs, F, 1, 1] + [bs, F, 1, 1] * ( [bs, F, 1, 1] - [bs, 1, 1, 1] )
        # / [bs, 1, 1, 1]
        out = alphas * (x - mu) / (s + self.eps) + betas + \
            gammas * (mu - m) / (v + self.eps)  # [bs, F, h, w]
