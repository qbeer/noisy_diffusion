...
if __name__ == '__main__':
    ...
    for ax, sample in zip(axes, samples):
        ...
        ax.set_yticks([])
    plt.tight_layout()
    plt.savefig('samples/noisy_cifar_samples.png', dpi=50)
    plt.close()

    fig, axes = plt.subplots(3, 3, sharex=True, sharey=True, figsize=(10, 10))
    axes = axes.flatten()
    for ax, sample in zip(axes, samples):
        ax.imshow(sample['image'].permute(1, 2, 0).numpy(), vmin=0, vmax=1)
        ax.set_title(f"Label : {sample['label']}")
        ax.set_xticks([])
        ax.set_yticks([])
    plt.tight_layout()
    plt.savefig('samples/cifar_samples.png', dpi=50)
